...
def get_image_array(img):
    """Convert a PIL Image to a NumPy array (RGBA)."""
...
def refine_shape(base_img, target_img, shape,
    ...
    best_shape, best_diff = simulated_annealing_shape(
        base_img, target_img, best_shape,
...
# -----------------------------------------------
# Geometrize Function (runs the process and updates the Streamlit UI)
# -----------------------------------------------
...
def run_geometrize(target_img, shape_type, shape_count, resize_factor,
    ...
    # Convert to RGBA and optionally resize
    ...
    # Create Streamlit placeholders for the image and progress text
    img_placeholder = st.empty()
    progress_placeholder = st.empty()

    # Run the shape addition loop
...
st.title("Geometrize Image App")

st.write("Upload an image, select a shape type, set the number of shapes and resize factor, then click **Generate** to create a geometrized image in real time.")
...
if uploaded_file is not None:
    ...
    # 3. Enter number of shapes (default 300)
    shape_count = st.number_input("Number of shapes", min_value=1, value=300, step=1)

    # 4. Slider for resize factor (between 0.25 and 0.5)
    resize_factor = st.slider("Resize factor (lower factor means faster generation but low quality)", 0.25, 0.5, 0.5, step=0.01)
    ...
    if st.button("Generate"):
        ...
        st.download_button("Download", byte_im, file_name="geometrized.png", mime="image/png")
